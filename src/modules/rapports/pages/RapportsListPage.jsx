      ...
      <div className="space-y-5">
        ...
        <Card padding={false}>
          {filtered.length === 0 ? (
            ...
            <>
              {/* Header */}
              <div className="hidden sm:grid sm:grid-cols-12 gap-4 px-5 py-3 border-b border-slate-100">
                {[
                  ...
                  <span key={h.label} className={`text-xs font-semibold text-slate-400 uppercase tracking-wide ${h.span}`}>
                    {h.label}
                  </span>
                ))}
              </div>
              ...
              <div className="divide-y divide-slate-50">
                {paginated.map((r) => (
                  <div key={r.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 sm:gap-4 px-5 py-4 hover:bg-slate-50/60 transition-colors items-center">
                    <div className="col-span-4">
                      <p className="text-sm font-semibold text-slate-800">{r.chantier}</p>
                      <p className="text-xs text-slate-400">{r.auteur}</p>
                      <p className="text-xs text-slate-500 mt-0.5 line-clamp-1 hidden sm:block">{r.travaux}</p>
                    </div>
                    <div className="col-span-2 text-sm text-slate-600">{formatDate(r.date)}</div>
                    <div className="col-span-2 flex items-center gap-1.5 text-xs text-slate-500">
                      <Cloud size={13} className="text-slate-400" />{r.meteo}
                    </div>
                    <div className="col-span-2 flex items-center gap-1.5 text-sm text-slate-600">
                      <Users size={13} className="text-slate-400" />{r.effectif}
                    </div>
