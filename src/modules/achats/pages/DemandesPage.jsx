              ...
              <tbody className="divide-y divide-gray-50">
                {demandes.map((d) => (
                  <tr key={d.id} className="hover:bg-gray-50">
                    <td className="py-3 px-4 font-mono text-xs text-gray-500">{d.reference}</td>
                    <td className="py-3 px-4 font-medium text-gray-800 max-w-xs">
                      <p className="truncate">{d.article}</p>
                    </td>
                    <td className="py-3 px-4 text-gray-500">{d.chantier}</td>
                    <td className="py-3 px-4 font-semibold text-gray-700">{formatCurrency(d.montant)}</td>
                    <td className="py-3 px-4 text-gray-500">{d.demandeur}</td>
                    ...
                    <td className="py-3 px-4">
                      {d.status === 'en_attente' && (
                        <div className="flex gap-1">
                          <button onClick={() => handleApprove(d)} className="p-1.5 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition-colors">
                            <Check size={14} />
                          </button>
