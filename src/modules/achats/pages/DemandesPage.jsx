...
export default function DemandesPage() {
  ...
  const onSubmit = (data) => {
    dispatch(addDemande({
      ...
      demandeur: user?.name ?? 'Chef de Chantier',
      status: 'en_attente',
    }));
  ...
  // CDT valide le besoin (étape 1)
  const handleValidateCDT = (d) => {
    dispatch(updateDemande({ ...d, status: 'valide_cdt' }));
    toast.success('Besoin validé — en attente approbation budget DAF');
  };

  // DAF approuve le budget (étape 2)
  const handleValidateDAF = (d) => {
    dispatch(updateDemande({ ...d, status: 'approuve' }));
    toast.success('Budget approuvé — commande autorisée');
  };

  // GST réceptionne la livraison (étape 3)
  const handleReceive = (d) => {
    dispatch(updateDemande({ ...d, status: 'livre' }));
