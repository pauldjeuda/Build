...
import {
  DollarSign, HardHat, FileText, AlertTriangle, TrendingUp, Users,
} from 'lucide-react';
...
const activity = [
  { text: 'Rapport journalier soumis — Immeuble Akwa', time: 'il y a 5 min',  color: 'bg-blue-500'    },
  { text: 'Demande achat approuvée — 10 M FCFA ciment', time: 'il y a 1h',   color: 'bg-emerald-500' },
  { text: 'Alerte stock — Sable de rivière sous seuil', time: 'il y a 2h',   color: 'bg-amber-500'   },
    ...
    <div className="bg-white border border-slate-100 rounded-xl p-3 shadow-lg text-xs">
      ...
      {payload.map((p) => (
        <div key={p.name} className="flex items-center gap-2">
          ...
          <span className="text-slate-500">{p.name === 'ca' ? 'CA' : 'Dépenses'} :</span>
          <span className="font-medium text-slate-700">{p.value} M FCFA</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          ...
          <Card className="lg:col-span-3">
            <CardHeader
              ...
              subtitle="5 derniers mois — en millions FCFA"
            />
            <ResponsiveContainer width="100%" height={210}>
              <BarChart data={caData} barSize={20} barGap={4}>
                ...
                <Bar dataKey="ca"  fill="#2563EB" radius={[4, 4, 0, 0]} name="ca"  />
                <Bar dataKey="dep" fill="#F59E0B" radius={[4, 4, 0, 0]} name="dep" />
              </BarChart>
            </ResponsiveContainer>
          </Card>
                ...
                <div key={c.nom}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex-1 min-w-0 mr-2">
                      <p className="text-xs font-semibold text-slate-700 truncate">{c.nom}</p>
                      <p className="text-xs text-slate-400">{c.chef}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <Badge variant={statusVariant(c.status)} dot>{statusLabel(c.status)}</Badge>
                      <span className="text-xs font-bold text-slate-700 w-8 text-right">{c.avancement}%</span>
                    </div>
                  </div>
        ...
        <Card>
          ...
          <div className="space-y-0 divide-y divide-slate-50">
            {activity.map((a, i) => (
              <div key={i} className="flex items-center gap-4 py-3 hover:bg-slate-50/50 transition-colors rounded-xl px-1 -mx-1">
